...
- **Semantic search:** runs entirely in-process, no database — BM25 candidate retrieval (`backend/app/services/search_lexical.py`) narrows the ~1600-document corpus, then a local cross-encoder reranker ([fastembed](https://github.com/qdrant/fastembed), ONNX Runtime, no PyTorch/API key) does the real relevance judgment
...
That's it — the map, incident list, filters, grouped-marker clicks, **and semantic search** all work with just this command, no API key and no database required. The first search after a fresh start takes a few extra seconds while the cross-encoder reranker model downloads once (cached after that); everything else is available immediately. If it doesn't come up, see [Troubleshooting](#troubleshooting).
...
Editing the CSVs takes effect on the next restart automatically — search runs directly over the in-memory `DataStore`, so there's no separate reindex/ingest step.
...
Semantic search works out of the box here too — no database, no extra setup. For AI-found sources, set `GOOGLE_API_KEY`. See `backend/.env.example` for the full list of variables.
...
A working free-tier combination: **Vercel or Netlify** for the frontend and **Render** (free web service) for the backend. No database is needed — semantic search runs entirely in-process (BM25 + a local cross-encoder reranker), which is also what keeps the backend within Render's 512MB free-tier memory limit. Any equivalent free static host / container host works the same way.
...
The service listens on port 8001 (see `backend/Dockerfile`). The cross-encoder reranker runs on `fastembed` (ONNX Runtime, no PyTorch), which is intentionally chosen over `sentence-transformers` to stay well under memory-capped free tiers like Render's 512MB instance — importing PyTorch alone can eat 150-300MB before a model is even loaded.
...
- **Search box shows "keyword matches only" instead of semantic results** — the reranker model is likely still downloading on its first use (check `docker compose logs api` for progress), or the API container has no outbound internet access for that one-time download. The rest of the app keeps working either way.
...
| GET | `/api/v1/search` | Semantic search over incidents/cables (no API key or database needed; runs in-process) |
